...
trait UploadFileTrait
{
     ...
     * This can be defined in the config file. If uploaded.memory_limit is false/null then it won't do anything.
    ...
    protected function increaseMemoryLimit()
    {
        ...
        if (config("uploaded.memory_limit")) {
            @ini_set('memory_limit', config("uploaded.memory_limit"));
        }
    }
     ...
     */
    protected function getImageFilename(string $suggested_title = null, $image_size_details, UploadedFile $photo)
    {
        if (empty($suggested_title)) {
            $suggested_title = uuid();
        }
        ...
        for ($i = 1; $i <= self::$num_of_attempts_to_find_filename; $i++) {
            ...
            $attempt = slug($base . $suffix . $wh) . $ext;
    ...
    protected function image_destination_path()
    {
        if (!$this->destinationPath) {
            $this->destinationPath = public_path('/' . config("uploaded.upload_dir"));
        }

        $this->check_image_destination_path_is_writable($this->destinationPath);
     ...
     * @throws \Exception
     */
    protected function UploadAndResize($suggested_title, $image_size_details, $photo, $destination = null)
    {
        ...
        if (is_array($image_size_details)) {
            ...
            if (isset($image_size_details['crop']) && $image_size_details['crop']) {
                $resizedImage = $resizedImage->resizeDown($w, $h);
            } else {
        ...
        // save image
        $resizedImage->save($destinationPath . '/' . $image_filename, config("uploaded.image_quality", 80));
    ...
    protected function getWhForFilename($image_size_details)
    {
        if (is_array($image_size_details)) {
            ...
            return "-" . slug(substr($image_size_details, 0, 30));
        }
