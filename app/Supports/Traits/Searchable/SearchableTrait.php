...
namespace App\Supports\Traits\Searchable;
...
trait SearchableTrait
{
    ...
    {
        $query = (array)($query ?: array_not_null(Request::except(['page', 'pageSize', 'order', 'sort'])));
            ...
            if ($field == 'search') {
                if (!empty($value)) {
                    $searchable = collect($this->_getSearchableAttributes($builder))->filter(function ($request) {
                        return !str_contains($request, ':') && !str_contains($request, '*');
                    })->toArray();
                    if (empty($searchable)) {
                        $searchable =  $builder->getModel()->getFillable();
                    }
                    $builder->where(function (Builder $builder) use ($searchable, $value) {
    ...
    protected function getConstraints(Builder $builder, array $query)
    {
        ...
        foreach ($query as $field => $values) {
            if ($this->isFieldSearchable($builder, $field) && !empty($values) && $field != 'search') {
