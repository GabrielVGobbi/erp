...
export default function SuppliersIndex() {
    ...
    const columns = React.useMemo<ColumnDef<Supplier>[]>(() => [
        ...
        createSortableColumn<Supplier>('Nome', 'name'),
        createSortableColumn<Supplier>('CPF / CNPJ', 'cpf_cnpj'),
        createAdvancedActionsColumn<Supplier>({
            groups: [
                {
                    label: 'Visualização',
                    actions: [
                        {
                            ...
                            type: 'link',
                            href: (inventory: Supplier) => route('suppliers.show', inventory.id)
                        },
                        {
                            label: 'Histórico',
                ...
                },
                {
                    ...
                    actions: [
                        ...
                        },
                        {
                            ...
                            type: 'action',
                            variant: 'secondary',
                            condition: (inventory: Supplier) => inventory.status === 'inactive',
                            onClick: async (inventory: Supplier, onRefresh) => {
                                try {
                                    await router.patch('/');
                        ...
                        },
                        {
                            ...
                            type: 'action',
                            variant: 'secondary',
                            condition: (inventory: Supplier) => inventory.status === 'active',
                            onClick: async (inventory: Supplier, onRefresh) => {
                                try {
                                    await router.patch('/');
                                    if (onRefresh) onRefresh();
                    ...
                    actions: [
                        {
                            ...
                            onClick: async (inventory: Supplier, onRefresh) => {
                                try {
                                    await api.delete(route('api.v2.suppliers.destroy', inventory.id));
                                    if (onRefresh) {
                                        onRefresh();
                                    }
                                } catch (error) {
                ...
                <div className="space-y-6">
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <DataTableServer
                            ...
                            storageKey="suppliers-table-bulk"
